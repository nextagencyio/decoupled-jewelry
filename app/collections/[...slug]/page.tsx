...
async function getCollection(path: string): Promise<DrupalCollection | null> {
  try {
    ...
    const { data } = await client.raw(GET_COLLECTION_BY_PATH, { path })
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/collections/${slug.join('/')}`
  const item = await getCollection(path)

  if (!item) {
    return { title: 'Collection Not Found | Lumina Fine Jewelry' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  {(item as any).season && (
                    <div>
                      <dt className="text-sm text-gray-500">Season</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).season}</dd>
                    </div>
                  )}
