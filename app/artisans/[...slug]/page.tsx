...
import { GET_ARTISAN_BY_PATH } from '@/lib/queries'
import { DrupalArtisan } from '@/lib/types'
...
interface ArtisanByPathData {
  route: {
    entity: DrupalArtisan
  } | null
}
...
async function getArtisan(path: string): Promise<DrupalArtisan | null> {
  try {
    ...
    const { data } = await apolloClient.query<ArtisanByPathData>({
      query: GET_ARTISAN_BY_PATH,
      variables: { path },
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const path = `/artisans/${slug.join('/')}`
  const item = await getArtisan(path)

  if (!item) {
    return { title: 'Artisan Not Found | Lumina Fine Jewelry' }
  }
            ...
            <div className="lg:col-span-1">
              <div className="bg-white rounded-xl shadow-sm p-6 sticky top-24">
                ...
                <dl className="space-y-4">
                  ...
                  {(item as any).origin && (
                    <div>
                      <dt className="text-sm text-gray-500">Training Origin</dt>
                      <dd className="font-semibold text-gray-900">{(item as any).origin}</dd>
                    </div>
                  )}
                </dl>
                <div className="mt-8">
